...
export const createIncidencia = async (req, res) => {
  ...
  try {
    ...
    console.log(zone)
    const newIncidencia = new IncidenciaMessage({...incidencia, address: address.data.results[1].formatted_address, idZone: zone.idZone, zoneName: zone.name, color: zone.color ? zone.color : "red", status: 0, createdAt: new Date().toISOString()});
